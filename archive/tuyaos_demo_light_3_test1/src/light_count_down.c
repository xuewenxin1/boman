...
/************  回调函数 ***/
/* 三路灯：主灯 + 夜灯，无辅灯。memory: 2=主灯, 4=夜灯 */
...
STATIC VOID countdown_restore_light_on(VOID_T)
{
    ...
    } else if (sg_demo_info.rhythm_switch && !sg_rhythm_interrupted) {
        /* 记忆主灯：按当前节律亮/色温开，不打断节律 */
...
STATIC VOID my_countdown_over_cb(VOID_T)
{
    ...
    if (sg_demo_info.night_switch == 0 && sg_demo_info.white_switch == 0)
    {
        countdown_restore_light_on();
    }
    else
    {
        TUYA_PWM_NUM_E all_channels[3];
        UINT32_T all_duties[3];
