...
- **短断电**（中断脚检测到，走 `power_check_timer_cb`）：打断今日节律并上报 `RHYTHM_STATUS=1`，不做上电节律恢复。
- **长断电**（冷启动，未走中断脚短断电回调）：默认约 **60s** 上电渐变恢复到当前节律；若时间同步后当前时段应为关灯，则改为打断节律（保留当前灯态）。
- 上电全量同步按 `sg_rhythm_interrupted` 上报 `RHYTHM_STATUS`，避免写死成 0。
- App/遥控器开灯、调光仍走条件打断接口。
- 关灯（App/遥控总关）：只停渐变、灯保持关，**不打断节律**；灯关着时不改灯面，节律线程仍在跑。
- 再总开/开主灯（App、遥控总开、主灯键）：节律未打断则立刻用当前节律亮/色温，不先用旧记忆值起渐变。
- **开夜灯**（记忆开灯 / 夜灯开关 / 智能夜灯 / 遥控夜灯）：打断今日节律并上报，后续调光调色节律不再运行。
