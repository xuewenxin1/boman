...
STATIC VOID boot_ms_print(CONST CHAR_T *tag)
{
    ...
    if (sg_boot_t0_ms == 0) {
        sg_boot_t0_ms = (now == 0) ? 1 : now;
        if (now == 0) {
            now = 1;
        }
    }
    tkl_log_output("[BOOT] %s now=%ums from_app_main=%ums\r\n",
                   tag, now, now - sg_boot_t0_ms);
}
...
VOID ty_app_wf_nw_stat_cb(IN CONST GW_WIFI_NW_STAT_E stat)
{
    ...
    TAL_PR_DEBUG("stat:%d, first_network:%d", stat, sg_demo_info.first_network);
...
STATIC VOID_T user_main(VOID_T)
{
    ...
    // tkl_log_output("second = %ld\r\n",tal_system_get_second());
...
#endif
{
    sg_boot_t0_ms = (UINT32_T)tal_system_get_millisecond();
    if (sg_boot_t0_ms == 0) {
        sg_boot_t0_ms = 1;
    }
    tkl_log_output("[BOOT] tuya_app_main called now=%ums\r\n", sg_boot_t0_ms);
