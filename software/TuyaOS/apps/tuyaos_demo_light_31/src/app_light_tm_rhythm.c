...
static void set_light_state_separate(uint8_t white_bright, uint8_t white_temp)
{
    ...
    if ((white_bright == 0 && sg_demo_info.white_switch)) {
        ...
        // 更新总开关状态
        if ((sg_demo_info.white_switch == 0) && (sg_demo_info.night_switch == 0)) {
    ...
    // 【关键修改】检查是否有任何灯亮着
    bool any_light_on = sg_demo_info.white_switch || sg_demo_info.night_switch;
    ...
    if (white_bright > 0) {
        ...
        if (sg_demo_info.white_switch || !any_light_on) {
            ...
            // TAL_PR_NOTICE("设置主灯状态：亮度=%d, 色温=%d", sg_demo_info.white_bright, sg_demo_info.white_temp);
...
{
    set_light_state_separate(bright, temp);
}
...
{
    extern DEMO_INFO_T sg_demo_info;
    bool result = (sg_demo_info.white_switch || sg_demo_info.night_switch);
    return result;
}
// 修改auto_light_on_action函数：
void auto_light_on_action(RHYTHM_NODE_DATA_T *node, int node_pos, int enabled_nodes[], int enabled_count, int actual_fade_minutes)
{
    ...
    // 重要：如果目标亮度为0，直接返回，不执行自动开灯
    if (target_bright == 0) {
        // TAL_PR_NOTICE("自动开灯：目标亮度为0，不执行自动开灯");
        return;
    }
    ...
    uint8_t white_switch_target = 1;
    // switch (sg_demo_info.last_light_memory) {
    //     case 2: white_switch_target = 1; break; // 只开主灯
    //     // case 3: white_switch_target = 0; break; // 只开辅灯
    //     // default: white_switch_target = 1; break;
    // }
    ...
    // TAL_PR_NOTICE("自动开灯：开关目标 - 主灯=%d,亮度目标 - 主灯=%d",white_switch_target, white_target_bright);
                                        ...
                                        uint8_t *prev_bright, uint8_t *prev_temp)
{
    ...
    *prev_temp = NO_CHANGE_FLAG;
    ...
    if (*prev_bright == NO_CHANGE_FLAG) {
        // 如果灯是关着的，使用默认值1
        if (sg_demo_info.white_switch ) {
            *prev_bright = sg_demo_info.white_bright;
        } else {
...
static void process_fade_update(void)
{
    ...
    if (elapsed_time >= sg_fade_state.duration_ms) {
        ...
        // 设置最终值
        set_light_state_separate(sg_fade_state.white_target_bright,sg_fade_state.white_target_temp);
        ...
        // 设置当前灯光状态
        set_light_state_separate(current_white_bright,current_white_temp);
...
{
    if (!white_bright || !white_temp) return;
...
static void handle_power_on_recovery(void)
{
    ...
    // 只有主灯或辅灯开启时才需要恢复
    if (!sg_demo_info.white_switch && !sg_demo_info.night_switch) {
        return;
    }
    ...
    // 计算节律目标值
    uint8_t white_target_bright, white_target_temp;
    ...
    if ((white_bright_diff > 5 || white_temp_diff > 5) && sg_demo_info.white_switch) {
        ...
        // 直接设置节律值
        set_light_state_separate(white_target_bright, white_target_temp);
        rhythm_persist_light_state(true);
    }
}
...
void apply_correct_rhythm_settings(bool calculate_only)
{
    ...
    if (fade_minutes == 0) {
        ...
        // 如果后节点时间是第二天的（即next_node_time > 1440）
        if (next_node_time >= 24 * 60) {
            // 跨天情况：后一个节点是第二天的
            if (current_total >= prev_node_time) {
                // 当前时间在前一个节点之后，但在第二天后一个节点之前
                use_next_node = false;
            } else {
                // 当前时间在第二天后一个节点之后
...
static void *rhythm_thread(void *arg)
{
    ...
    for (;;) {
        ...
        extern POSIX_TM_S local_tm;
        int current_total = local_tm.tm_hour * 60 + local_tm.tm_min;
        // TAL_PR_NOTICE("sg_rhythm_interrupted  =%d\r\n",sg_rhythm_interrupted);
        // ============ 节律打断检查 ============
        if (sg_rhythm_interrupted) {
            // 如果渐变正在进行，停止它
            if (sg_fade_state.active) {
            ...
            if (enabled_count >= 2) {
                ...
                if (should_recover && target_node_idx != -1) {
                    ...
                    app_light_schedule_before_rhythm_start();
                    // 执行恢复逻辑
                    //TAL_PR_NOTICE("在节点%d的渐变开始时间恢复节律\n", target_node_idx);
                    ...
                    if (target_node_pos != -1) {
                        ...
                        if (target_fade_minutes > 0) {
                            ...
                            //TAL_PR_NOTICE("恢复渐变：从当前状态向节点%d渐变\n", target_node_idx);
                            //TAL_PR_NOTICE("起始值：亮度=%d, 色温=%d\n", start_white_bright, start_white_temp);
                            //TAL_PR_NOTICE("目标值：亮度=%d, 色温=%d\n", target_white_bright, target_white_temp);
                            //TAL_PR_NOTICE("渐变时长：%d分钟\n", target_fade_minutes);
                            ...
                            // 立即变化 - 只设置目标值
                            //TAL_PR_NOTICE("立即设置到节点%d的值\n", target_node_idx);
                            set_light_state_separate(target_white_bright, target_white_temp);
                            rhythm_persist_light_state(true);
                        }
                    }
...
void app_light_tm_rhythm_syn(void)
{
    extern DEMO_INFO_T sg_demo_info;
    if (!sg_demo_info.rhythm_switch || sg_rhythm_interrupted || 
        !(sg_demo_info.white_switch ||  sg_demo_info.night_switch)) {
        return;
    }
    ...
    for (int i = 0; i < enabled_count; i++) {
        ...
        if (node->auto_light_on == 1) {
            ...
            if (time_diff == 0 || time_diff == -1) { 
                ...
                // TAL_PR_NOTICE("自动开灯触发: 节点%d时间=%02d:%02d, auto_on_time=%02d:%02d, time_diff=%d, 实际渐变=%d分钟",
                            //  enabled_nodes[i], node->hour, node->minute,
                            //  auto_on_time/60, auto_on_time%60, time_diff, actual_fade_minutes);
                return true;
            }
        }
    }
