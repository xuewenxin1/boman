    ...
    for (index = 0; index < dp_cnt; index++)
    {
        ...
        case DPID_SWITCH:
            ...
            if (sg_demo_info.switch_status == 1)
            {
                // ==================== 开灯逻辑 ====================
                // 检查是否应该进入夜灯模式
                if ((app_nightlight_should_enter()) && (sg_demo_info.night_switch == 0 && sg_demo_info.white_switch == 0))
                {
                ...
                else
                {
                    ...
                    extern bool sg_rhythm_interrupted;
                    bool rhythm_follow = (sg_demo_info.rhythm_switch && !sg_rhythm_interrupted && !open_night);
        ...
        case DPID_WORK_MODE:
            ...
            {
                TUYA_PWM_NUM_E channels[] = {BRIGHT_PWM, TEMP_PWM,NIGHT_BRIGHT_PWM};
                UINT32_T duties[] = { LIGHT_PWM_BOUNDARY_MAX, LIGHT_PWM_BOUNDARY_MIN, 0 };
                pwm_gradual_duty_set_multi(3, channels, duties);
                sg_demo_info.white_switch = 1;
                sg_demo_info.switch_status = 1;
                sg_demo_info.white_bright = 100;
        ...
        case DPID_WHITE_BRIGHT:
...

            {
                ...
                TAL_PR_NOTICE("sg_demo_info.white_bright1 = %d\r\n",sg_demo_info.white_bright);
        ...
        case RHYTHM_SWITCH:
        {
            ...
            if (sw)
            {
                ...
                // 检查是否有主灯或辅灯开启
                bool main_light_on = (sg_demo_info.white_switch ||  sg_demo_info.night_switch);
        ...
        case LIGHT_SWITCH:
            ...
            if (dp_data->value.dp_bool == 1)
            {
                TAL_PR_NOTICE("sg_demo_info.white_bright2 = %d\r\n",sg_demo_info.white_bright);
                sg_demo_info.switch_status = 1;
                upload_device_bool_status(LIGHT_SWITCH, 1);
                upload_device_bool_status(DPID_SWITCH, 1);
                if (sg_demo_info.night_switch == 1)
        ...
        case DPID_RESET:
        {
            ...
            if (dp_data->value.dp_bool == 1)
            {
                ...
                sg_demo_info.white_switch = 1;
                sg_demo_info.white_bright = 100;
                sg_demo_info.last_light_memory = 2; /* light_3 仅主灯，重置后默认主灯记忆 */
                sg_demo_info.white_temp = 55;
                sg_demo_info.night_switch = 0;
                sg_demo_info.night_bright = 50;
...
{
    if (!p) return FALSE;
    if (p->time_stamp == 0 || p->time_stamp == 0xFFFFFFFF) 
        return FALSE;
    uint8_t data = (p->checksum == __demo_calc_checksum(p));
    return data;
}
                          ...
                          BOOL_T check_calibration, uint8_t cal_value)
{
    ...
    while (cur + sizeof(DEMO_INFO_T) <= end) {
        ...
        /* 全 0xFF：追加区结束。坏洞（非空且校验失败）：跳过继续扫，避免丢掉后面的 cal=3 */
        if (__demo_is_blank_slot(&tmp)) {
            break;
        }
...
int device_config_backup_to_c(void)
{
    ...
    /* C 块只存校准备份；未校准不写，避免无意义擦写 */
    if (sg_demo_info.calibration != 3) {
        TAL_PR_NOTICE("Backup: skip C, calibration=%d (need 3)", sg_demo_info.calibration);
        return -8;
    }
...
void __demo_flash_init(void)
{
    ...
    if (!found_valid) {
        ...
        /* A/B 都扫描，取 time_stamp 更大的一条（避免只认 A、丢掉 B 上更新的 cnt=0） */
        if (__read_hdr(BLOCK_A, &ha)) {
            DEMO_INFO_T tmp = {0};
            UINT_T tmp_addr = 0;
            if (__scan_block(BLOCK_A, &tmp, &tmp_addr, FALSE, 0)) {
                /* 仅接受 cal==0/3，排除中间态 1/2 */
    ...
    if (found_valid) {
        ...
        // 无有效运行数据：只擦 A/B 重建；C 留给校准备份，不碰
        TAL_PR_NOTICE("Init: No valid data, erase A/B then defaults (C untouched)");
...
int device_config_save1(void)
{
    ...
    if (sg_last_save1_ms != 0) {
        ...
        if (elapsed < SAVE1_COOLDOWN_MS) {
            ...
            /* 冷却结束后只补写 1 次最新 RAM，避免 Flash 写满 */
