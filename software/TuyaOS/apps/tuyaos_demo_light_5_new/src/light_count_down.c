...
STATIC VOID countdown_restore_light_on(VOID_T)
{
    ...
    if (open_night) {
    ...
    } else if (sg_demo_info.rhythm_switch && !sg_rhythm_interrupted && (open_white || open_aux)) {
        /* 记忆主/辅/主辅：按当前节律亮/色温开，不打断 */
        upload_device_bool_status(LIGHT_SWITCH, open_white ? 1 : 0);
        upload_device_bool_status(AUX_SWITCH, open_aux ? 1 : 0);
        ...
        uint16_t ww = 0, cw = 0, aux_ww = 0, aux_cw = 0;
