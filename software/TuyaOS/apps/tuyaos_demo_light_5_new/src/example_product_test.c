...
#define PRODUCT_TEST_AGING_MAIN        0x01    // 主辅光老化
#define PRODUCT_TEST_AGING_NIGHT       0x02    // 夜灯老化
#define PRODUCT_TEST_AGING_WW          0x03    // 主辅光WW路100%老化
#define PRODUCT_TEST_AGING_CW          0x04    // 主辅光CW路100%老化
...
STATIC VOID __start_aging_test(PRODUCT_TEST_TYPE_E aging_type)
{
    ...
    if(aging_type == PRODUCT_TEST_AGING_MAIN) {
        ...
        // 设置主辅光老化模式：主光CW路50%亮度，WW路50%亮度，辅光CW路50%亮度，WW路50%亮度
        ...
        uint16_t aux_bright1 = 5 + (100 - 1) * 95 / 99;
        uint8_t aux_temp1 = 50;
...
#endif
        tkl_pwm_duty_set(AUX_BRIGHT_PWM, calibration_num * aux_ww/10000);
...
#endif
        tkl_pwm_duty_set(AUX_TEMP_PWM, calibration_num * aux_cw/10000);
        ...
        calibration_num = sg_demo_info.aux_ww_calibration_coefficient;
#endif
        tkl_pwm_duty_set(AUX_BRIGHT_PWM, calibration_num);
        tkl_pwm_start(AUX_BRIGHT_PWM);
        tkl_pwm_duty_set(AUX_TEMP_PWM, 0);
        tkl_pwm_start(AUX_TEMP_PWM);
        tkl_pwm_duty_set(NIGHT_BRIGHT_PWM, 0);
        ...
        calibration_num = sg_demo_info.aux_cw_calibration_coefficient;
#endif
        tkl_pwm_duty_set(AUX_TEMP_PWM, calibration_num);
        tkl_pwm_start(AUX_TEMP_PWM);
        tkl_pwm_duty_set(NIGHT_BRIGHT_PWM, 0);
        tkl_pwm_start(NIGHT_BRIGHT_PWM);
    }
