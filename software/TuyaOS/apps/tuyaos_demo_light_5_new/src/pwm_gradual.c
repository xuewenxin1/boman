...
static void stop_current_and_set_immediate(UINT8_T channel_count, TUYA_PWM_NUM_E channels[], UINT32_T duties[])
{
        ...
        for (int i = 0; i < global_ctrl.channel_count; i++)
        {
            ...
            if (ch < PWM_CH_MAX)
            {
                ...
                if (!found)
                {
                    ...
                    // if(ch == AUX_BRIGHT_PWM)
                    //     calibration_num = sg_demo_info.aux_ww_calibration_coefficient;
                    // if(ch == AUX_TEMP_PWM)
                    //     calibration_num = sg_demo_info.aux_cw_calibration_coefficient;
                    #if load_calibration
                        extern int test_main_load;
                        extern int test_auxiliary_load;
                    #endif
                    uint16_t calibration_num = 10000;
                    if(ch == BRIGHT_PWM){
                        #if load_calibration
                            calibration_num = sg_demo_info.ww_calibration_coefficient*test_main_load/100;
                        #else
                            calibration_num = sg_demo_info.ww_calibration_coefficient;
                        #endif
                    }
                    if(ch == TEMP_PWM){
                        #if load_calibration
                            calibration_num = sg_demo_info.cw_calibration_coefficient*test_main_load/100;
                        #else
                            calibration_num = sg_demo_info.cw_calibration_coefficient;
                        #endif
                    }
                    if(ch == AUX_BRIGHT_PWM){
                        #if load_calibration
                            calibration_num = sg_demo_info.aux_ww_calibration_coefficient*test_auxiliary_load/100;
                        #else
                            calibration_num = sg_demo_info.aux_ww_calibration_coefficient;
                        #endif
                    }
                    if(ch == AUX_TEMP_PWM){
                        #if load_calibration
                            calibration_num = sg_demo_info.aux_cw_calibration_coefficient*test_auxiliary_load/100;
                        #else
                            calibration_num = sg_demo_info.aux_cw_calibration_coefficient;
                        #endif
                    }
    ...
    for (int i = 0; i < channel_count; i++)
    {
        ...
        if (ch < PWM_CH_MAX)
        {
            ...
            //     calibration_num = sg_demo_info.cw_calibration_coefficient;
            // if(channels[i] == AUX_BRIGHT_PWM)
            //     calibration_num = sg_demo_info.aux_ww_calibration_coefficient;
            // if(channels[i] == AUX_TEMP_PWM)
            //     calibration_num = sg_demo_info.aux_cw_calibration_coefficient;
            #if load_calibration
                extern int test_main_load;
                extern int test_auxiliary_load;
...
static VOID_T pwm_gradual_timer_cb(VOID_T *arg)
{
    ...
    if (elapsed >= global_ctrl.gradual_time)
    {
        ...
        for (int i = 0; i < global_ctrl.channel_count; i++)
        {
            ...
            if (ch < PWM_CH_MAX)
            {
                ...
                uint16_t num = calibration_num*global_ctrl.target_values[i]/10000;
                // tkl_log_output("ch = %d,current_value = %ld\r\n",ch,calibration_num*global_ctrl.target_values[i]/10000);
                if(num != 0){
    ...
    for (int i = 0; i < global_ctrl.channel_count; i++)
    {
        ...
        if (current_value != channel_state[ch].current_value)
        {
            ...
            // if(ch == BRIGHT_PWM || ch == TEMP_PWM){
            //     if(num <= 3)
            //         num = 3;
            // }
            // if(ch == AUX_BRIGHT_PWM || ch == AUX_BRIGHT_PWM){
            //     if(num <= 1)
            //         num = 1;
            // }
            // tkl_log_output("ch = %d,current_value = %ld\r\n",ch,calibration_num*current_value/10000);
            if(num != 0){
                tkl_pwm_duty_set(ch, num);
                tkl_pwm_start(ch);
            }
...
OPERATE_RET pwm_gradual_duty_set_multi_ex(UINT8_T channel_count, TUYA_PWM_NUM_E channels[], UINT32_T duties[], UINT8_T curve_type)
{
    ...
    // TAL_PR_NOTICE("global_ctrl.gradual_time = %d\r\n",global_ctrl.gradual_time);
    // 启动定时器
...
OPERATE_RET pwm_gradual_close_channels_keep_rest(UINT8_T close_count, TUYA_PWM_NUM_E close_chs[], UINT32_T close_duties[])
{
    ...
    if (close_count == 0 || close_count > MAX_CHANNELS_PER_REQUEST || !close_chs || !close_duties) {
        return OPRT_INVALID_PARM;
    }
    if (!timer_initialized) {
        OPERATE_RET ret = pwm_gradual_init();
    ...
    TUYA_PWM_NUM_E keep_chs[MAX_CHANNELS_PER_REQUEST];
    UINT32_T keep_start[MAX_CHANNELS_PER_REQUEST];
    UINT32_T keep_target[MAX_CHANNELS_PER_REQUEST];
    UINT8_T keep_count = 0;
    UINT32_T remain_ms = 0;
    UINT8_T curve_type = global_ctrl.curve_type;

    if (global_ctrl.is_active) {
        UINT32_T now = tkl_system_get_millisecond();
        UINT32_T elapsed = (now >= global_ctrl.start_time) ? (now - global_ctrl.start_time) : 0;
        if (elapsed < global_ctrl.gradual_time) {
            remain_ms = global_ctrl.gradual_time - elapsed;
        }
        for (int i = 0; i < global_ctrl.channel_count; i++) {
            TUYA_PWM_NUM_E ch = global_ctrl.channels[i];
            BOOL_T is_close = FALSE;
            for (UINT8_T j = 0; j < close_count; j++) {
                if (close_chs[j] == ch) {
                    is_close = TRUE;
                    break;
                }
            }
            if (!is_close && keep_count < MAX_CHANNELS_PER_REQUEST) {
                keep_chs[keep_count] = ch;
                keep_start[keep_count] = channel_state[ch].current_value;
                keep_target[keep_count] = global_ctrl.target_values[i];
                keep_count++;
            }
        }
        global_ctrl.is_active = FALSE;
    ...
    /* 关闭通道立刻落到目标（通常为 0） */
    for (UINT8_T i = 0; i < close_count; i++) {
        TUYA_PWM_NUM_E ch = close_chs[i];
        if (ch >= PWM_CH_MAX) {
            continue;
        }
        channel_state[ch].current_value = close_duties[i];
        ...
        if (ch == BRIGHT_PWM) {
            ...
            calibration_num = sg_demo_info.cw_calibration_coefficient;
        } else if (ch == AUX_BRIGHT_PWM) {
            calibration_num = sg_demo_info.aux_ww_calibration_coefficient;
        } else if (ch == AUX_TEMP_PWM) {
            calibration_num = sg_demo_info.aux_cw_calibration_coefficient;
        }
        UINT32_T num = (UINT32_T)calibration_num * close_duties[i] / 10000;
        if (num != 0) {
            tkl_pwm_duty_set(ch, num);
            tkl_pwm_start(ch);
        } else {
    ...
    /* 其余通道从当前值继续渐到原目标 */
    if (keep_count > 0 && remain_ms > 0) {
        BOOL_T need = FALSE;
        for (UINT8_T i = 0; i < keep_count; i++) {
            if (keep_start[i] != keep_target[i]) {
                need = TRUE;
                break;
            }
        }
        if (need) {
            global_ctrl.is_active = TRUE;
            global_ctrl.start_time = tkl_system_get_millisecond();
            global_ctrl.gradual_time = remain_ms;
            if (global_ctrl.gradual_time < 20) {
                global_ctrl.gradual_time = 20;
            }
            global_ctrl.channel_count = keep_count;
            global_ctrl.curve_type = curve_type;
            for (UINT8_T i = 0; i < keep_count; i++) {
                global_ctrl.channels[i] = keep_chs[i];
                global_ctrl.start_values[i] = keep_start[i];
                global_ctrl.target_values[i] = keep_target[i];
            }
            start_gradual_timer();
            return OPRT_OK;
        }
