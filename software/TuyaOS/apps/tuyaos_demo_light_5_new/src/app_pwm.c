        ...
        if (sg_demo_info.cnt1 == 1)
        {
            ...
            case 1:{
                ...
                if(sg_demo_info.switch_status == 0 ||
                   (sg_demo_info.white_switch == 0 && sg_demo_info.aux_switch == 0 && sg_demo_info.night_switch == 0)){
                    /* 先清再恢复，避免残留通道导致 App 显示与实灯不一致 */
                    ...
                    else if(sg_demo_info.last_light_memory == 4){
...
all_channels[channel_count] = AUX_BRIGHT_PWM;
                ...
                else if (sg_demo_info.aux_switch)
...
all_channels[channel_count] = BRIGHT_PWM;
                ...
                if (sg_demo_info.switch_change_gear[4] == 1)
                {
                    sg_demo_info.aux_switch = 1;
                    sg_demo_info.switch_status = 1;
                    sg_demo_info.aux_bright = sg_demo_info.switch_change_gear[5];
                    sg_demo_info.white_temp = sg_demo_info.switch_change_gear[6];
                    uint16_t aux_bright1 = 5 + (sg_demo_info.aux_bright - 1) * 95 / 99;
                    uint8_t aux_temp1 = sg_demo_info.white_temp;
                    uint16_t aux_ww = aux_bright1 * aux_temp1;
...
all_channels[channel_count] = AUX_BRIGHT_PWM;
            ...
            case 3:{
                ...
                if (sg_demo_info.switch_change_gear[16] == 1)
                {
                    sg_demo_info.aux_switch = 1;
                    sg_demo_info.switch_status = 1;
                    sg_demo_info.aux_bright = sg_demo_info.switch_change_gear[17];
                    sg_demo_info.white_temp = sg_demo_info.switch_change_gear[18];
                    uint16_t aux_bright1 = 5 + (sg_demo_info.aux_bright - 1) * 95 / 99;
                    uint8_t aux_temp1 = sg_demo_info.white_temp;
                    uint16_t aux_ww = aux_bright1 * aux_temp1;
...
all_channels[channel_count] = AUX_BRIGHT_PWM;
        ...
        /* 仅在校准有效时落盘，避免校准失效态（强制夜灯）固化到 Flash */
        device_config_save1();
    }
    else{
        ...
        TUYA_PWM_NUM_E channels[] = {BRIGHT_PWM, TEMP_PWM, AUX_BRIGHT_PWM, AUX_TEMP_PWM, NIGHT_BRIGHT_PWM};
        UINT32_T duties[] = {0, 0, 0, 0, 100*100};
        pwm_gradual_duty_set_multi(5, channels, duties);
    }
    return;
}
...
/**
 * @brief 复位/配网后固定出光：主+辅亮度100、色温55，夜灯关
 */
...
VOID_T app_led_init()
{
    ...
    extern uint16_t gradual_time_ms;//渐变时间
    extern int pwm_gradual;//配网渐变是否完成
    ...
    UINT32_T duties4[] = {3, 3, 3, 3};
    pwm_gradual_duty_set_multi(4, channels4, duties4);

    pwm_gradual_wait_current_complete(1000);  // 等待1秒
    TUYA_PWM_NUM_E channels5[] = {BRIGHT_PWM, TEMP_PWM, AUX_BRIGHT_PWM, AUX_TEMP_PWM, NIGHT_BRIGHT_PWM};
    UINT32_T duties5[] = LIGHT_PWM_BLINK_DUTIES5_INIT;
    pwm_gradual_duty_set_multi(5, channels5, duties5);

    pwm_gradual_wait_current_complete(1000);  // 等待1秒
    TUYA_PWM_NUM_E channels6[] = {BRIGHT_PWM, TEMP_PWM, AUX_BRIGHT_PWM, AUX_TEMP_PWM};
    UINT32_T duties6[] = {3, 3, 3, 3};
