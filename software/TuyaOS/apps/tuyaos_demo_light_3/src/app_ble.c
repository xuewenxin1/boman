...
VOID __update_pwm_output(UINT8_T light_type)
{
    ...
    //             white_temp1 = 1;
    //         uint16_t ww = white_bright1 * white_temp1;
    //         uint16_t cw = white_bright1 * (101 - white_temp1);
    //         if (ww >= 9997)
    //         {
    //             ww = 9997;
    //             cw = 3;
...
STATIC VOID __handle_single_press_brightness(UINT8_T light_type, BOOL_T is_increase)
{
    ...
    case LIGHT_TYPE_MAIN:
        if (sg_demo_info.white_switch == 1)
        {
            // uint8_t current_level = __get_current_brightness_level(sg_demo_info.white_bright);
 ...
 */
STATIC VOID __reverse_calculate_from_pwm(UINT16_T ww_pwm, UINT16_T cw_pwm, UINT8_T adjust_type,
...
STATIC VOID __handle_key_event(REMOTE_PARSE_RESULT_T *result)
{
    ...
    case KEY_ID_POWER: // K3 总开/关
        if (result->operation_type == OP_TYPE_SHORT_PRESS)
        {
            ...
            if ((app_nightlight_should_enter()) &&(sg_demo_info.white_switch == 0&&sg_demo_info.night_switch == 0))
            {
            ...
            else if ((sg_demo_info.night_switch == 0) && (sg_demo_info.white_switch == 0))
            {
                
                dp_report = 1;
                uint8_t memory_state = sg_demo_info.last_light_memory;
                extern bool sg_rhythm_interrupted;
                bool rhythm_follow = (sg_demo_info.rhythm_switch && !sg_rhythm_interrupted);
                ...
                case 4: // 夜灯记忆：开夜灯并打断节律
                    ...
                    sg_demo_info.switch_status = 1;
                    sg_demo_info.white_switch = 0;
                    TUYA_PWM_NUM_E channels_n[] = {BRIGHT_PWM, TEMP_PWM, NIGHT_BRIGHT_PWM};
                    UINT32_T duties_n[] = {0, 0, sg_demo_info.night_bright * 100};
                    pwm_gradual_duty_set_multi(3, channels_n, duties_n);
    ...
    case KEY_ID_MODE: // K25 模式
        ...
        if (result->operation_type == OP_TYPE_SHORT_PRESS)
        {
            ...
            case 0x12:                  // 黄光
                ...
                sg_demo_info.night_switch = 0;
                TUYA_PWM_NUM_E channels2[] = {BRIGHT_PWM, TEMP_PWM, NIGHT_BRIGHT_PWM};
                UINT32_T duties2[] = {5500, 4500, 0};
                pwm_gradual_duty_set_multi(3, channels2, duties2);
                dp_report = 1;
                break;
            ...
            case 0x13:                  // 中性光
                ...
                UINT32_T duties3[] = {60 * 35, 60 * 65 ,0};
    ...
    case KEY_ID_MAIN_SWITCH: // 主灯开/关（3路禁用，用总开关）
        break;
    ...
    case KEY_RESET: // 配网复位
        if (result->operation_type == OP_TYPE_LONG_PRESS)
        {
            ...
            sg_demo_info.white_bright = 100;
            sg_demo_info.white_temp = 55;
            sg_demo_info.last_light_memory = 2; /* light_3 仅主灯 */
            sg_demo_info.night_switch = 0;
...
VOID upload_device_all_status1(VOID_T)
{
    ...
    memset((UCHAR_T *)p_all_obj_dp, 0, 6 * SIZEOF(TY_OBJ_DP_S));
    ...
    rt = dev_report_dp_json_async(NULL, p_all_obj_dp, 6);
}
