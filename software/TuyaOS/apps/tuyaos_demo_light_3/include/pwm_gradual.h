...
// 渐变请求结构
typedef struct {
    TUYA_PWM_NUM_E channels[3];      // 最多5个通道
    UINT32_T target_values[3];       // 对应目标值
...
typedef struct {
    ...
    UINT32_T start_values[3];        // 起始值
    UINT32_T target_values[3];       // 目标值
    TUYA_PWM_NUM_E channels[3];      // 通道列表
    UINT8_T channel_count;           // 通道数量
    
    // 队列
    GRADUAL_REQUEST_T queue[16];     // 请求队列
