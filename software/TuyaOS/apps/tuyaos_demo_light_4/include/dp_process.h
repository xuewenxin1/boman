...
#define HAND_SWEEP_SWITCH         114
#define POWER_OUTAGE              117
...
typedef struct {
    ...
    uint8_t aux_switch;             // 辅助灯开关
    uint8_t aux_bright;             // 辅助灯亮度
    // uint8_t white_temp;               // 辅助灯色温
    ...
    uint8_t beep_switch;             // 蜂鸣器开关
    uint8_t hand_sweep_switch;        // 手势开关
    uint8_t calibration;            //校准
    uint16_t ww_calibration_coefficient;
    uint16_t cw_calibration_coefficient;
    uint16_t aux_ww_calibration_coefficient;
    uint16_t aux_cw_calibration_coefficient;
    // 数组字段
...
// STATIC_ASSERT(sizeof(DEMO_INFO_T) == 221, "DEMO_INFO_T size mismatch");
/***********************************************************
