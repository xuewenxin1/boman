...
uint8_t night[6] = {0x00, 0x1, 0x17, 0x00, 0x06, 0x00};
...
uint8_t power_outage[10] = {
    0x00,0x00,0x07,0xEA,0x01,0x01,0x00,0x00,0x00,0x00};
STATIC VOID_T __soc_dev_reset_inform_cb(GW_RESET_TYPE_E type)
{
    ...
    if (type == GW_REMOTE_UNACTIVE || type == GW_LOCAL_UNACTIVE)
    {
        /* SDK 已在做解绑，此处只清业务标志，禁止再调 unactive/reset 以免配网异常 */
        sg_demo_info.first_network = 0;
        device_config_save1();
    }
    else if (type == GW_REMOTE_RESET_FACTORY || type == GW_LOCAL_RESET_FACTORY ||
...
VOID ty_app_wf_nw_stat_cb(IN CONST GW_WIFI_NW_STAT_E stat)
{
    ...
    // TAL_PR_NOTICE("stat = %d,sg_demo_info.first_network = %d\r\n",stat,sg_demo_info.first_network);
...
#ifdef ENABLE_WIFI_SERVICE
    TUYA_CALL_ERR_RETURN(tuya_iot_wf_soc_dev_init(GWCM_LOW_POWER, WF_START_AP_FIRST, &iot_cbs, PID, USER_SW_VER));
#ifdef ENABLE_WIRED
    // init wired linkage
...
STATIC VOID_T user_main(VOID_T)
{
    ...
    // };
    // TUYA_CALL_ERR_LOG(tkl_gpio_init(BEEP_PIN, &out_pin_cfg));
    TUYA_PWM_BASE_CFG_T pwm_cfg = {
        .duty = 0, /* 1-10000 */
        .frequency = 2700,
        .polarity = TUYA_PWM_NEGATIVE,
    };
    tkl_pwm_init(BEEP_PWM,&pwm_cfg);
