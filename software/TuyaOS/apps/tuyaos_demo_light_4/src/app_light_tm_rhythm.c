                                     ...
                                     uint8_t aux_bright, uint8_t aux_temp)
{
    ...
    // TAL_PR_NOTICE("set_light_state_separate: 主灯(bright=%d, temp=%d), 辅灯(bright=%d, temp=%d)", white_bright, white_temp, aux_bright, aux_temp);
    ...
    // 【关键修改】检查是否有任何灯亮着
    bool any_light_on = sg_demo_info.white_switch || sg_demo_info.aux_switch ;
    ...
    if (aux_bright > 0) {
        ...
        if (sg_demo_info.aux_switch || !any_light_on) {
            ...
            // TAL_PR_NOTICE("设置辅灯状态：亮度=%d, 色温=%d", sg_demo_info.aux_bright, sg_demo_info.white_temp);
...
{
    extern DEMO_INFO_T sg_demo_info;
    bool result = (sg_demo_info.white_switch || sg_demo_info.aux_switch );
    return result;
}
...
void auto_light_on_action(RHYTHM_NODE_DATA_T *node, int node_pos, int enabled_nodes[], int enabled_count, int actual_fade_minutes)
{
    ...
    // TAL_PR_NOTICE("自动开灯：开关目标 - 主灯=%d, 辅灯=%d；亮度目标 - 主灯=%d, 辅灯=%d",white_switch_target, aux_switch_target, white_target_bright, aux_target_bright);
