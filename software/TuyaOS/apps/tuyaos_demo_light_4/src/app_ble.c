...
STATIC UINT8_T sg_remote_color_cycle_idx = 0; /* 遥控四档光色本地序号，与 payload 无关 */
STATIC UINT8_T sg_last_mode_param = 0xFF; // 上一帧模式键 payload，相同则视为按住连发

#define REMOTE_COLOR_PRESET_COUNT 4
...
STATIC VOID __handle_single_press_brightness(UINT8_T light_type, BOOL_T is_increase)
{
    ...
    extern BOOL_T sg_wake_is_timing;
    extern DEMO_INFO_T sg_demo_info;
    // extern uint32_t white_off_time;
    // extern uint32_t aux_off_time;
            app_light_schedule_user_interrupt_sleep_wake();
...
/**
 * @brief 遥控四档光色（4路PWM，与 App 情景独立，不上报 work_mode 0~3）
 * @param preset_idx 0日光 1阅读 2就餐 3温馨
 */
STATIC VOID __remote_apply_color_preset(UINT8_T preset_idx)
{
    extern DEMO_INFO_T sg_demo_info;
    TUYA_PWM_NUM_E channels[4] = {BRIGHT_PWM, TEMP_PWM, AUX_BRIGHT_PWM, AUX_TEMP_PWM};
    UINT32_T duties[4] = {0, 0, 0, 0};

    sg_demo_info.white_switch = 1;
    sg_demo_info.aux_switch = 1;
    sg_demo_info.switch_status = 1;

    switch (preset_idx % REMOTE_COLOR_PRESET_COUNT) {
    case 0: /* 日光：主+辅 100%，色温 100% */
...
STATIC VOID __handle_key_event(REMOTE_PARSE_RESULT_T *result)
{
    ...
    extern uint32_t g_main_off_time;
    extern uint32_t g_aux_off_time;
    extern BOOL_T sg_sleep_is_timing;
    extern BOOL_T sg_wake_is_timing;
    ...
    UINT32_T current_time = tal_system_get_millisecond();
    BOOL_T mode_param_changed = (remote_data->key_id == KEY_ID_MODE &&
                                 result->operation_type == OP_TYPE_SHORT_PRESS &&
                                 result->additional_param != sg_last_mode_param);

    // 防抖检查（模式键 payload 变化视为新一次按下，不受同键防抖限制）
    ...
    case KEY_ID_POWER: // K3 总开/关
        if (result->operation_type == OP_TYPE_SHORT_PRESS)
        {
            ...
            if ((app_nightlight_should_enter()) &&(sg_demo_info.white_switch == 0&&sg_demo_info.aux_switch == 0))
            {
            ...
            else if ((sg_demo_info.aux_switch == 0) && (sg_demo_info.white_switch == 0))
            {
                upload_device_enum_status(DPID_WORK_MODE, 16); // 正常模式
                dp_report = 1;
                // 正常开灯模式
            ...
            else
            {
                ...
                else if (sg_demo_info.white_switch == 1)
                {
                    ...
                    g_main_off_time = tal_system_get_millisecond();
                    if(g_main_off_time - g_aux_off_time <= 5000)
                       sg_demo_info.last_light_memory = 1;
                    TUYA_PWM_NUM_E channels[] = {BRIGHT_PWM, TEMP_PWM};
                    UINT32_T duties[] = {0, 0};
                    pwm_gradual_duty_set_multi(2, channels, duties);
                ...
                {
                    sg_demo_info.aux_switch = 0;
                    g_aux_off_time = tal_system_get_millisecond();
                    if(g_aux_off_time - g_main_off_time <= 5000)
                       sg_demo_info.last_light_memory = 1;
    ...
    case KEY_ID_MODE: // K25 四档光色：payload 变化切下一档，与 App 情景独立
        if (sg_demo_info.rhythm_switch == 1) {
            app_light_stop_today_rhythm_timer();
        }
            app_light_schedule_user_interrupt_sleep_wake();

        upload_device_enum_status(DPID_WORK_MODE, 16); /* App 面板不跟随遥控模式键 */
        if (result->operation_type == OP_TYPE_SHORT_PRESS)
        {
            UINT8_T mode_param = result->additional_param;

            /* 与上一帧相同 = 按住连发同一条数据，不切换 */
            if (mode_param == sg_last_mode_param) {
                break;
            }
            sg_last_mode_param = mode_param;

            sg_last_key_id = 0;
            __remote_apply_color_preset(sg_remote_color_cycle_idx);
            sg_remote_color_cycle_idx = (sg_remote_color_cycle_idx + 1) % REMOTE_COLOR_PRESET_COUNT;
            dp_report = 1;
        }
        break;
    case KEY_ID_MODE1:
        ...
        if (result->operation_type == OP_TYPE_LONG_PRESS) {
            ...
            if (sg_long_press_active && sg_long_press_key == KEY_COLLECT) {
                ...
                if (long_press_elapsed >= COLLECT_LONG_PRESS_TIME) {
                    ...
                    UINT32_T duties1[] = {9999, 3, 9999, 3};
    ...
    case KEY_ALIGNMENT: // 对码
        if (result->operation_type == OP_TYPE_LONG_PRESS)
        {
            ...
            {
                all_channels[channel_count] = BRIGHT_PWM;
                all_duties[channel_count++] = sg_demo_info.white_bright * sg_demo_info.white_temp;

                all_channels[channel_count] = TEMP_PWM;
                all_duties[channel_count++] = sg_demo_info.white_bright * 100 - (sg_demo_info.white_bright * sg_demo_info.white_temp);
            }
            else
            {
                all_channels[channel_count] = BRIGHT_PWM;
            ...
            {
                all_channels[channel_count] = AUX_BRIGHT_PWM;
                all_duties[channel_count++] = sg_demo_info.aux_bright * sg_demo_info.white_temp;

                all_channels[channel_count] = AUX_TEMP_PWM;
                all_duties[channel_count++] = sg_demo_info.aux_bright * 100 - (sg_demo_info.aux_bright * sg_demo_info.white_temp);
            }
            else
            {
                all_channels[channel_count] = AUX_BRIGHT_PWM;
